...
public class SecretCode {
	...
	//@ public invariant (\forall int x;  x>=0 && x<5; code[x].length()== 1); 
	...
	//@ requires (\forall int x; x>=0 && x<5; c[x].length()==1);
	// inizialmente i controlli sono settati a falso
	//@ ensures (\forall int x;  x>=0 && x<5; previous[x]==false);
	...
	//@ requires (\forall int x; x>=0 && x<5; (c[x].length()== 1) && c[x]!=null);
	...
	//@ ensures \result <==> (\forall int x; x>=0 && x<5; code[x].equals(c[x]) || !previous[x]);
	//@ ensures (\result == false) <== !(\exists int x; x>=0 && x<5; !(code[x].equals(c[x])) || previous[x]);
	//@ diverges true;
	...
	//@ ensures  (\forall int x;  x>=0 && x<5; code[x].equals(c[x])) <==> \result==10*turn;
	// se ho fatto qualcosa di sbagliato, devo aver guadagnato meno di 10*turn punti
	//@ ensures  (\exists int x;  x>=0 && x<5; !(code[x].equals(c[x]))) ==> \result<=10*turn;
	//@ diverges true;
	...
	//@ ensures  (\forall int x; x>=0 && x<code.length; !(code[x].equals(l))) <==> !\result;
	//@ diverges true;
